...
const PARTICLES = Array.from({ length: 20 }, (_, index) => ({
  ...
  startX: (index * 67) % 960,
  startY: (index * 41) % 640,
  endX: (index * 103 + 180) % 960,
  endY: (index * 59 + 120) % 640,
  size: 2 + (index % 4),
  hue: 200 + ((index * 11) % 60),
...
const renderTechFallback = (name, size) => (
  <div
    style={{
      width: size,
      height: size,
...
export function TechIcon({ name, size = 24 }) {
  const renderer = TECH_ICON_RENDERERS[normalizeTechName(name)];
  return renderer ? renderer({ size }) : renderTechFallback(name, size);
}
  ...
  return (
    <div className="animated-shapes">
      ...
      <Motion.div
        ...
        animate={{ rotate: [360, 0], x: [0, 100, 0], y: [0, -50, 0] }}
...
export function SuccessIcon({ size = 60 }) {
  return (
    <Motion.svg
      width={size}
      height={size}
...
export function ProjectExternalIcon({ size = 16 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
...
export function ProjectGithubIcon({ size = 16 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
